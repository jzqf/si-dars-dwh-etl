...
CREATE TABLE public.obo__analysis_queue (
   ...
   analysis_queue_status_timestamp  TIMESTAMP            null,
   analysis_queue_type_id           SMALLINT             null,
   analysis_queue_priority_class_id SMALLINT             null,
   analysis_request_id              BIGINT               null,
   pedd_id                          BIGINT               null,
   charged_road_section_id          BIGINT               null,
   image_view_id                    SMALLINT             null,
   shuffle_id                       INTEGER              null,
   queued_by_sid                    BIGINT               null,
   queued_by_euid                   VARCHAR(20)          null,
   queued_on                        TIMESTAMP            null,
   fetched_by_sid                   BIGINT               null,
   fetched_by_euid                  VARCHAR(20)          null,
   fetched_on                       TIMESTAMP            null,
   queued_for_group_id              SMALLINT             null,
   queued_for_user_sid              BIGINT               null,
   queued_for_user_euid             VARCHAR(20)          null,
   exclude_group_id                 SMALLINT             null,
   exclude_user_sid                 BIGINT               null,
   exclude_user_euid                VARCHAR(20)          null,
